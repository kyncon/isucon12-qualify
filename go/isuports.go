...
func playersAddHandler(c echo.Context) error {
	...
	insertPlayers := make([]PlayerRow, 0, len(displayNames))
	...
	for _, displayName := range displayNames {
		...
		now := time.Now().Unix()
		insertPlayers = append(insertPlayers, PlayerRow{
			ID:             id,
			TenantID:       v.tenantID,
			DisplayName:    displayName,
	...
	if _, err := tenantDB.NamedExecContext(
		...
		return fmt.Errorf("error Insert player at tenantDB bulk!!: %w; %v", err, insertPlayers)
	}
...
func playerHandler(c echo.Context) error {
	...
	for _, ps := range pss {
		...
		if ok != true {
			fmt.Printf("competitionID: %v, competitionMap:%v", ps.CompetitionID, competitionMap)
